...
#' properly formatted \code{\link[base]{list}} (see details) in active graphics window.
...
#' properly formatted \code{\link[base]{list}}
#' @param clstmethod Determines the heatmap clustering algorithm "kmeans" for k-means (default, see \code{\link[stats]{kmeans}}), 
#'  "hclust" (see \code{\link[stats]{hclust}}) for hierarchical clustering, "ssom" for 
#'  (super) self organising map (see \code{\link[kohonen]{supersom}}) with torus topology and "none"
#'  of FALSE to turn off the clustering
#' @param clusters The number of cluster for "kmeans" and "hclust", ignored for "ssom", defaults to 5L
#' @param ssomt1 Determines , the dimensionality of SOM - number of neurons in 1st dimension,
#'  number of resulting clusters equals ssomt1*ssomt2, defaults to 2L
#' @param ssomt2 Determines , the dimensionality of SOM - number of neurons in 2st dimension,
#'  number of resulting clusters equals ssomt1*ssomt2, defaults to 2L
#' @param include The logical vector indicating if given subplot should influence 
#' clustering and sorting, if given element is FALSE the sub-heatmap will be still plotted,
#' and the order of data rows will be determined by clustering/sorting other sub-heatmaps, 
...
#' @param main The main title of the plot, shown in top-centre part of the figure; defaults to NULL (not visible)
#' @param labels The character vector giving sub-titles of heatmaps (plotted over the heatmap and below the main title). 
#'  The defaults NULL value indicates that feature/track file names will be used to generate the sub-titles.
#' @param ord The numeric vector determining the plotting order of experiments. 
#'  The heatmap representing feature-track pair with the highest priority will be 
...
#' @param zmin global minimum value on colour key, ignored if \code{autoscale} is TRUE
#' @param zmax global maximum value on colour key, ignored if \code{autoscale} is TRUE
...
#' @param s The saturation value used to auto scale colour key limits, defaults to 0.01 
#' @param indi If TRUE (defaults) the independent colour keys will be plotted below heatmaps,
#'  if FALSE the commmon colour key is shown rightmost
...
#' @param colorspace The colours pace of the heatmap, see \code{\link[grDevices]{grDevices}}
