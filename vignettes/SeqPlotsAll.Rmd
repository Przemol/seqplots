...
<!-- 
%\VignetteIndexEntry{SeqPlots GUI vignette HTML version}
...
scripts([subproject home]) or as an R package ([subproject home]).
SeqPlots can also be deployed on a server using free and open sourced
(GPL licensed) Shiny Server (https://github.com/rstudio/shiny-server),
making it available for multiple clients. For the server version, clone
this repo to Shiny Server application directory and set up data
location:
`git clone https://przemol@bitbucket.org/przemol/seqplots.git`.
...
Please visit [**issues tracker**](../issues) to view currently know
issue. To report new issue/bug/feature request please click
[**here**](../issues/new). If issue is connected to file upload please
attach the file in the form.
...
framework server code [R] and client browser code/libraries [HTML5, CSS3
and JavaScript]. There are two derivative projects for desktop end
users:

-   [SeqPlots OSX App](https://bitbucket.org/przemol/seqplots_osxapp)
-   [SeqPlots Bioconductor R package]()
...
1.  Start the SeqPlots. Refer to installation guides for platform
    ...
    browser](../inst/seqplots/www/help/img//02_01.png "Optional title")
...
2.  Upload some feature (BED or GFF) and track (BigWig or WIG) files.
    ...
    > ![File upload panel](../inst/seqplots/www/help/img//02_02.png)
...
3.  When all is done press blue "Start upload" button. After upload and
    ...
    information please refer to [**errors
    chapter**](Errors%20explained).

    > ![File upload progress infoermation](../inst/seqplots/www/help/img//02_03.png)
...
4.  Dismiss upload window and press blue "New plot set" button on side
    ...
    > ![File management panel](../inst/seqplots/www/help/img//02_04.png)
...
5.  When you decided which files/motifs to plot it is time to set up the
    ...
    to learn more check [**this**](Running%20the%20plot-set%20jobs)
    section.

    > ![Plot set calculation options](../inst/seqplots/www/help/img//02_05.png)
...
6.  After options are set up press blue "Run calculation" button. This
    ...
    file to the plot-set).

    > ![Plot set calculation progress window](../inst/seqplots/www/help/img//02_06.png)
...
7.  After some time the calculation will finish (fingers crossed,
    ...
    plot. Choose one or more checkboxes and press grey "Replot" button
    (or RETURN from your keyboard). You can also check "Reactive
    plotting" checkbook just next to it - it will automatically apply
    changes to plots as soon you make them.

    > ![The plot selection grid](../inst/seqplots/www/help/img//02_07.png)

8.  Congratulation! Your First plot is complete, you can see the preview
    of it on the side panel.

    > ![The plot preview panel](../inst/seqplots/www/help/img//02_08.png)

9.  You are able to set up labels, titles, font sizes, legends and may
    more on side panel tabs, take a look at
    [**this**](Viewing%20and%20manipulating%20plots) chapter.

    > ![Plot settings tabs](../inst/seqplots/www/help/img//02_09.png)

10. By clicking the plot preview you can enlarge it for better view.
    When everything is ready you can get the plot as PDF bt clicking
    green "Line plot" button just on the top of side panel.

    ![Average plot example](../inst/seqplots/www/help/img//02_10.png)
...
11. You can also visualize the signal as a heatmap. Please note that
    ...
    > ![Heatmap settings tab](../inst/seqplots/www/help/img//02_11.png)
...
12. Similarly to line plot you can grab the heatmap PDF using 'Heatmap'
    ...
    ![Heatmap example](../inst/seqplots/www/help/img//02_12.png)

Installation: Mac OS X app
==========================

> Mac OS X bundle is an easy way to use SeqPlots for Mac OS X users. It
> contains R binaries and packages that SeqPlost depends on.
...
> downloaded using graphical user interface.
...
1.  Downaload the installation disk image from
    http://bitbucket.org/przemol/seqplots/downloads/SeqPlots\_0.9.1.dmg
2.  Double-click on DMG file to mount the image
3.  Drag and drop SeqPlots.app to applications

    > ![DMG image mounted in OS X](../inst/seqplots/www/help/img//03_01.png)

Usage
-----

After starting SeqPlots the welcome screen displaying software version,
currently installed genomes and data folder location would appear.

> ![SeqPlots Mac OS X bindle - the welcome screen](../inst/seqplots/www/help/img//03_02.png)

This screen allows you to set up following options:

-   Change your data location folder (by default your home directory
    will be used)
-   Install new genomes. Using this option requires Internet connection.
    Corresponding genome packages are required before uploading the
    files for plotting. Full list of supported genomes is available
    here:
    http://www.bioconductor.org/packages/release/BiocViews.html\#\_\_\_BSgenome

    > ![Genomic packages installer window](../inst/seqplots/www/help/img//03_03.png)

Press **`START`** button to initiate SeqPlots. If initiation was
successful the user interface should open in your default web browser.

For further usage please refer to [quick start guide](Quick%20start) or
specific chapters of [documentation](..).

The window in the background allows you to asses if SeqPlots is running
properly and exit the application at any moment.

> ![SeqPlots Mac OS X bindle - the diagnostic window](../inst/seqplots/www/help/img//03_04.png)

The "moving" animation progress bar indicates that SeqPlots is running.
Press "Cancel" to stop it. Pressing "Details" will reveal small text
...
> operating system is Windows or Linux. It is also the recommended
> method for R users Mac OS X.
...
devtools::install_bitbucket(repo='SeqPlots_Rpackage', username='przemol', 
auth_user=readline('User?> '), password=readline('Password?> '), dependencies="Depends")
```
...
default web browser. For further usage please refer to [quick start
guide](Quick%20start) or specific chapters of [documentation](..).
...
instruction [here](Installation%20-%20R%20package)
...
`git clone https://przemol@bitbucket.org/przemol/seqplots.git`. The data
location can be set up with following R code:
...
-   JS color picker added for browsers, that do not support
    <select type="color"> i.e. Firefox (checked with modernizr.js
    library)
