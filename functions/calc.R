...
calc <- function(input.cov, d1=2000, d2=3000, desc="", tss=NULL, rm0=FALSE, cat4=NULL, ignore_strand=FALSE) {
	...
	if (ignore_strand) {
		...
		for (j in seqlevels(tss)) {
			...
			if (length(tss_chr) > 0) {
				M <- matrix(NA, ncol=length(-d1:d2), nrow=length(tss_chr))
				cov_chr <- input.cov[[j]]
				
				for (i in 1:length(tss_chr)) {
						try( M[i,] <-  as.numeric(cov_chr[ tss_chr[i] + (-d1:d2) ]), T )
				}
			...
			if (length(tss_chr) > 0) {
				...
				for (i in 1:length(tss_chr)) {
					if(as.logical(tss_strand[i] == "+")) {
						...
						try( M[i,] <-  as.numeric(cov_chr[ tts_chr[i] - (-d1:d2) ]), T )	
					}
				}
				cat(j, i, length(cov_chr), "\n")
					cat4(paste(j, ' [',length(cov_chr),'bp]: ', i,  ' features summarized.',sep=''))
