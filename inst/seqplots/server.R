...
#      )
  ...
  observe({
    ...
    if(length(query$addbw)){
        ...
        file_genome <- if(lenghh(query$bw_genome)) query$bw_genome else 'ce11'
        file_user	<- if(lenghh(query$bw_user)) query$bw_user else 'jadb'
        file_comment<- if(lenghh(query$bw_comment)) query$bw_comment else ''
