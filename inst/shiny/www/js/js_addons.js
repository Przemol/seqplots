            ...
            "fnRowDeselected": function ( node ) {
             ...
             {"sExtends": "text", "sButtonText": "Select filtered", "fnClick": function ( node, conf ) {
               ...
               if ( zz.length < 50 ) {
                 ...
                 alert('Select less than 50 rows!')
               }
...
sendToCalc = function() {
	...
	if( !(f_sel.length >= 1 & (t_sel.length >= 1 | $("#SFpattern").val().length >= 1)) ) {
...
rmSelctedFiles = function() {
	if($('input[name="f_tracks"]:checked').length > 0 | $('input[name="f_features"]:checked').length > 0) {
		if ( confirm("Are you sure?") ){
			Shiny.shinyapp.sendInput({"TR_delfile":new Date().getTime()}); 
			Shiny.shinyapp.sendInput({"TR_delfile":null});
		}
