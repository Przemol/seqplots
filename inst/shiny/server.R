...
doFileOperations <- function(x, final_folder='files', file_genome, file_user, file_comment) {
	...
	if( grepl('.(gff|GFF)$', x) ) {
		...
		if( !all(seqlevels(BigWigFile(x)) %in% seqlevels(gnm)) ) { stop('Unknown chr names in BigWiggle file, use UCSC compatible!') }
...
mcCalcStart <- quote({
            ...
            procSF(values$SFsetup, input$f_features,
                   ...
                   type=input$plot_type, rm0=input$rm0, ignore_strand=input$ignore_strand, 
                   add_heatmap=input$SFadvanced, cat3=cat3, cat4=cat4)
          } else if ( input$algo_type == 'Quick [track-at-once]') {
						procQuick(input$f_tracks, input$f_features,
							x1 = input$plot_upstream, xm = input$anchored_downstream, x2 = input$plot_downstream,
							type = input$plot_type, bin= as.numeric(input$BWbin),
							cat3=cat3, cat4=cat4, rm0=input$rm0, ignore_strand=input$ignore_strand, add_heatmap=input$add_heatmap)		
...
# 	}, deleteFile = TRUE)
	...
	values <- reactiveValues( grfile=NULL, calcID=NULL, calcMsg1=NULL, calcMsg2=NULL, script=NULL, plotMsg=NULL, refFileGrids=NULL, proc=NULL, im=NULL, clusters=NULL, include=NULL, SFsetup=list() )
  ...
  observe({   
    ...
    isolate({
      if(!  grepl( paste0('^[',paste(DNA_BASES,collapse=''),']+$'), toupper(input$SFpattern) ) ) stop('Use DNA letters only')
        #DNA_ALPHABET
  ...
  output$SFsetup  <- renderPrint({ str(values$SFsetup) })
  ...
  observe( mcDoParallel, quoted = TRUE, label = 'Plotting')
	...
	plotHeatmap <- function(pl) {
      ...
      clusts <- k$size
		...
		lab <- sapply(pl, '[[', 'desc')
    lab[values$lables!=''] <- values$lables[values$lables!='']
